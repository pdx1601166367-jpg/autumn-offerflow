...
(async () => {
  ...
  await step('resume', async () => {
    ...
    const resumeView = await page.textContent('#view');
    ok('resume no version panel', !resumeView.includes('版本记录'));
    ok('resume version button', await page.locator('[data-action="show-resume-versions"]').count() === 1);
    ok('resume result in right card', await page.locator('.card.panel', { hasText: '分析结果' }).locator('#resumeResult').count() === 1);
    await page.click('[data-action="show-resume-versions"]');
    await page.waitForSelector('.modal-head h3', { hasText: '简历版本' });
    ok('resume version modal', true);
  ...
  await step('resources link column', async () => {
    ...
    const resInfo = await page.evaluate(() => fetch('/api/resources', { cache: 'no-store' }).then(r => r.json()));
    const resItems = resInfo.items || [];
    ok('resources 2027 only', resItems.length >= 20 && resItems.every(x => String(x.title || '').includes('2027')) && resItems.every(x => !String(x.title || '').includes('2026') && !String(x.title || '').includes('春招')));
    ok('resources roles cities filled', resItems.every(x => x.roles && x.cities));
    ok('resources apply links official', resItems.every(x => x.link && x.link.indexOf('mp.weixin') < 0));
