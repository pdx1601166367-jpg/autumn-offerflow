(function () {
  ...
  const SOLVER_DB = [
    ...
    {
      key: "growth_exp", title: "设计一个增长实验", type: "产品", tags: ["增长", "实验"],
      hint: "先定目标和假设，再选实验设计与指标，最后给放量决策。",
      approach: "1. 明确实验目标（激活/留存/收入）和核心指标；2. 写出可证伪的假设（如：新手模板使首日完成率提升 10%）；3. 设计 A/B 分组、样本量、周期和护栏指标；4. 数据显著后评估是否放量；5. 沉淀实验结论到文档，避免重复试错。",
      code: null,
      complexity: "考察点：假设思维、实验设计、统计与决策"
    },
    {
      key: "metrics_board", title: "设计产品的指标体系与看板", type: "产品", tags: ["指标", "数据"],
      hint: "从北极星指标出发拆漏斗和分层指标，再看板落地。",
      approach: "1. 定义北极星指标与一级拆解（激活、留存、变现、传播）；2. 补充过程指标、护栏指标和诊断指标；3. 按用户分层与渠道维度交叉；4. 看板按「概览-漏斗-分层-诊断」组织，指标口径统一并注明负责人；5. 定期复盘异常波动。",
      code: null,
      complexity: "考察点：指标分层、口径管理、数据可视化"
    },
    {
      key: "ab_platform", title: "设计一个 A/B 实验平台", type: "产品", tags: ["AB实验", "平台"],
      hint: "覆盖实验创建、分流、监控、分析和归档全流程。",
      approach: "1. 实验创建：定义假设、指标、样本量和周期；2. 分流：随机分层、避免互斥冲突，支持多实验并行；3. 监控：显著性与护栏指标实时看板；4. 分析：给出结论建议与置信区间；5. 归档：沉淀实验文档和复用策略。",
      code: null,
      complexity: "考察点：平台化思维、统计严谨性、易用性"
    },
    {
      key: "recall", title: "设计沉默用户召回策略", type: "产品", tags: ["留存", "召回"],
      hint: "先定义沉默，再按原因分层触达，最后衡量回访与后续留存。",
      approach: "1. 定义沉默周期与分层（轻度/重度/流失预警）；2. 结合行为归因（低频、功能未用、价格敏感）；3. 设计触达渠道与权益（Push/短信/专属券/新功能唤醒）；4. 控制频次防骚扰；5. 衡量回访率、7 日留存和召回成本。",
      code: null,
      complexity: "考察点：用户分层、归因、触达治理与成本"
    }
  ];
